...
import datetime
import hashlib
from bootloader import db,redis_db
...
class User(db.Model):
    ...
    def __dict__(self):
        return {'id':self.id,'realname':self.realname,'mobile':self.mobile}
    class Meta:
        db_table = 'users'
...
def resetcategorys(model_class, instance,created):
    ...
    #session_manager.replace('categorys', categorys, 86400)
...
def resetdistributions(model_class, instance,created):
    ...
    #session_manager.replace('distributions', distributions, 86400)
